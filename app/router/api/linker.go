...
// 删
func DeleteLinker(c *gin.Context)  {
	g := util.Gin{C: c}
	m := &model.Linker{}
	if !util.IsAdmin(c) {
...
func GetLinkerById(c *gin.Context) {
	m := &model.Linker{}
	g := util.Gin{C: c}
	s := c.Param("id")
	id, _ := strconv.Atoi(s)
...
func GetLinkerPage(c *gin.Context) {
	var (
		page int
		err  error
		g    util.Gin
